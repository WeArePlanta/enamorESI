var wikishootme = {
	...
	colors : {
		...
		mnm_lc:'#D291BC',
		commons:'#62A9FF',
		commons_image_in_category:'#FFFF99',
	} ,
	map_is_set : false ,
	pos : { lat:52 , lng:0 } ,
	entries : {
...
//		me.tt.updateInterface($('#dropdownUploadsLi')) ;
	...
	loadWikimediaLayer : function ( server , mode ) {
		...
		me.loadCachedJSON ( api+'?callback=?' , {
			...
			if ( mode == 'commons' ) me.update_entries_commons_main_category() ;
...
//			console.log ( d ) ;
	...
	update_entries_commons_main_category : function () {
		let me = this ;
		$.each ( me.entries.commons , function ( entry_id , entry ) {
			let filename = entry.page ;
			if ( typeof me.files_in_main_commons_category[filename] == 'undefined' ) {
				entry.marker.setStyle ( {color:me.colors.commons, weight:1} ) ;
			} else {
				entry.marker.setStyle ( {color:me.colors.commons_image_in_category, weight:3} ) ;
			}
		} ) ;
	} ,

	clear_commons_main_category : function () {
		let me = this ;
